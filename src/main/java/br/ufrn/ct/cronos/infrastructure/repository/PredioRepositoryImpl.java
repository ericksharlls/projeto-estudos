...
import br.ufrn.ct.cronos.domain.repository.PredioRepositoryQueries;

import static br.ufrn.ct.cronos.infrastructure.repository.spec.PredioSpecs.porNome;
import static br.ufrn.ct.cronos.infrastructure.repository.spec.PredioSpecs.porDescricao;
...
@Repository
public class PredioRepositoryImpl implements PredioRepositoryQueries {
	...
	public List<Predio> findNomeAndDescricao(String nome, String descricao) {
		return predioRepository.findAll(porNome(nome).and(porDescricao(descricao)));
	}
    ...
    public Page<Predio> findComPaginacaoJPQLDinamico(String nome, String descricao, Pageable pageable) {
        ...
        //Page<Predio> prediosPage = new PageImpl<>(query.getResultList());
        
		//return query.getResultList();
        Page<Predio> prediosPage = new PageImpl<>(
                predios, 
                pageable, 
                getTotalCountJPQLDinamico(nome, descricao, pageable));

        return prediosPage;
    }
    
    private Long getTotalCountJPQLDinamico(String nome, String descricao, Pageable pageable) {
        var jpql = new StringBuilder();
		jpql.append("select COUNT(p.id) from Predio p where 0 = 0 ");
    ...
    public Page<Predio> findComPaginacaoCriteriaDinamico(String nome, String descricao, Pageable pageable) {
		...
		Page<Predio> prediosPage = new PageImpl<>(predios, 
                pageable, 
                getTotalCountCriteriaDinamico(builder, predicates.toArray(new Predicate[0])));

		return prediosPage;
    }

    private Long getTotalCountCriteriaDinamico(CriteriaBuilder criteriaBuilder, Predicate... predicateArray) {
