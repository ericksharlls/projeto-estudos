...
public class CadastroPredioIT {
	...
	/*
	@Test // teste do happy path (caminho feliz)
	public void deveAtribuirId_QuandoCadastrarPredioComDadosCorretos(){
		//cenário
		Predio novoPredio = new Predio();
		novoPredio.setNome("Prédio Legal");
		novoPredio.setDescricao("Descrição do Prédio Legal");
		
		//ação
		cadastroPredio.salvar(novoPredio);

		//validação
		assertThat(novoPredio).isNotNull();
		assertThat(novoPredio.getId()).isNotNull();
	}

	@Test // teste de um unhappy path (caminho infeliz)
	// outra possibilidade de nome para o método seria deveFalharAoCadastrarPredio_QuandoSemNome()
	public void deveFalhar_QuandoCadastrarPredioSemNomeOuDescricao(){
		// Cenário
		Predio novoPredio = new Predio();
		novoPredio.setNome(null);
		novoPredio.setDescricao("");
	...
	public void deveFalhar_QuandoExcluirPredioEmUso() {
		cadastroPredio.excluir(1L);
	}

	public void deveFalhar_QuandoExcluirPredioInexistente(){
		PredioNaoEncontradoException erroEsperado =
				Assertions.assertThrows(PredioNaoEncontradoException.class, () -> {
					cadastroPredio.excluir(1000L);
				});
